...
class ObjectNavAgent(Agent):
    ...
    def prepare_planner_inputs(
        ...
        print(f"2: obs size is {obs.shape}")
        (
            goal_map,
            found_goal,
            frontier_map,
    ...
    def act(self, obs: Observations):
        ...
        print(f"1: obs_preprocessed size is {obs_preprocessed.shape}")
    ...
    def _preprocess_obs(self, obs: Observations):
        ...
        if self.store_all_categories_in_map:
            ...
            depth_cp = copy.deepcopy(depth.detach().cpu()).squeeze(-1)
            depth_cp[obs.semantic != obs.task_observations["object_goal"]] = 10000
            x = np.min(depth_cp.numpy())
            print(f"min depth is {x}")

            if x < 60:
                print(f"stop in perception because min depth is {x}")
                sys.exit(0)
        ...
        if (
            ...
            start_recep_goal_category = torch.tensor(obj_goal_idx).unsqueeze(0)
        # start_recep_goal_category = None
        # if (
