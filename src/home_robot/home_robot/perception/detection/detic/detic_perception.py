...
import numpy as np
import torch
sys.path.append('/home/lxd/HomeRobot/home-robot/src/third_party/detectron2')
from detectron2.config import get_cfg
from detectron2.data import MetadataCatalog
from detectron2.engine.defaults import DefaultPredictor
from detectron2.utils.visualizer import ColorMode, Visualizer
from request_module import verify_target
...
class DeticPerception(PerceptionModule):
    ...
    def predict(
        ...
        if 1 in class_idcs:
            LLM_check = True
            self.detected_goal = 1
            print(f"\nDetection model thinks the {obs.task_observations['goal_name']} is contained in current image.")
        else:
            self.detected_goal = 0
            LLM_check = False
        if LLM_check:
            if self.llm_check==0:
                goal_contained = verify_target(rgb, obs.task_observations['goal_name'])
                # goal_contained=True
            else:
                goal_contained = True
            if goal_contained:
                self.llm_check=1
                print(f"\nMLLM thinks the {obs.task_observations['goal_name']} is contained in current image.")
            else:
                print(f"\nBut MLLM thinks the {obs.task_observations['goal_name']} is not contained in current image.")
        else:
        ...
        semantic_map, instance_map = overlay_masks(masks, class_idcs, (height, width), goal_contained, self.detected_goal)
