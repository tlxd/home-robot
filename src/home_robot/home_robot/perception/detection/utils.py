...
def overlay_masks(
    masks: np.ndarray, class_idcs: np.ndarray, shape: Tuple[int, int], goal_contained: bool, detected_goal: int
) -> np.ndarray:
    """Overlays the masks of objects
    Determines the order of masks based on mask size
    """
    ...
    for i_mask in sorted_mask_idcs[::-1]:  # largest to smallest
        ...
        if int(class_idcs[i_mask]) == 1:
            if goal_contained is False or detected_goal < 1:
                continue
