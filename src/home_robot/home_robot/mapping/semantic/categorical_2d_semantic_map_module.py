...
class Categorical2DSemanticMapModule(nn.Module):
    ...
    def __init__(
        ...
        self.camera_height = camera_height * 100.0
        self.agent_height = agent_height * 100.0
    ...
    def forward(
        ...
        print(f"3: obs size is {seq_obs.shape}")
    ...
    def _update_local_map_and_pose(  # noqa: C901
        ...
        print(f"5: obs size is {obs.shape}")
        ...
        # print(f"point_cloud_t shape is {point_cloud_t.shape}")
        ...
        point_cloud_base_coords = du.transform_camera_view_t(
            point_cloud_t, self.camera_height, torch.rad2deg(tilt).cpu().numpy(), device
        )

        # print(f"point_cloud_base_coords shape is {point_cloud_base_coords.shape}")


        # point_cloud_base_coords[0] = torch.tensor(observation.xyz) * 100
        ...
        if self.dilate_obstacles:
            self.obs_dilation_selem = skimage.morphology.disk(self.dilate_size)
            for i in range(fp_map_pred.shape[0]):
                env_map = fp_map_pred[i, 0].cpu().numpy()
                dilated_obstacles = cv2.dilate(env_map, self.obs_dilation_selem, iterations=self.dilate_iter)
                fp_map_pred[i, 0] = torch.tensor(dilated_obstacles)
