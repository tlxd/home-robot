...
from home_robot.agent.objectnav_agent.llm_guide import buildmap, LLMspatialreasoning
...
class ObjectNavFrontierExplorationPolicy(nn.Module):
    ...
    def reach_single_category(self, map_features, category, final_global_map, seq_lmb, seq_goal_name):
        ...
        goal_map = self.explore_otherwise(map_features, goal_map, found_goal, final_global_map, seq_lmb, seq_goal_name)

        return goal_map, found_goal

    def reach_object_recep_combination(
        self, map_features, object_category, recep_category, final_global_map, seq_lmb, seq_goal_name
    ):
        ...
        goal_map = self.explore_otherwise(map_features, goal_map, found_rec_goal, final_global_map, seq_lmb, seq_goal_name)
        return goal_map, found_goal
        ...
        if instance_id is not None:
            ...
            if nav_to_recep.sum() < map_features.shape[0]:
                goal_map_o, found_goal_o = self.reach_object_recep_combination(
                    map_features, object_category, start_recep_category, final_global_map, seq_lmb, seq_goal_name
                )
            # there is at least one instance in the batch where the goal is receptacle
                ...
                # )
                goal_map_r, found_goal_r = self.reach_single_category(
                    map_features, end_recep_category,final_global_map, seq_lmb, seq_goal_name
                )
            ...
            return self.reach_single_category(map_features, goal_category, final_global_map, seq_lmb, seq_goal_name)
    ...
    def explore_otherwise(self, map_features, goal_map, found_goal, final_global_map, seq_lmb, seq_goal_name):
        ...
        for e in range(batch_size):
            if not found_goal[e]:
                ...
                # if random.random() > 1.0:
                    ...
                    LLMresult = LLMspatialreasoning(buildmap(final_global_map), seq_lmb, seq_goal_name)
                    if LLMresult is not None:
                        goal_map[e] = copy.deepcopy(LLMresult)
                    else:
                        goal_map[e] = frontier_map[e]
