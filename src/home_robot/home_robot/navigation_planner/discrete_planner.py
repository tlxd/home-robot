...
class DiscretePlanner:
    ...
    def __init__(
        ...
        goal_tolerance: float = 1e-5, #0.01,
        ...
        Arguments:
            ...
            obs_dilation_selem_radius: radius (in cells) of obstacle dilation
             structuring element
            goal_dilation_selem_radius: radius (in cells) of goal dilation
             structuring element
    ...
    def plan(
        ...
        return self.next_action, action, closest_goal_map, short_term_goal, dilated_obstacles
    ...
    def get_action(
        ...
        if not (found_goal and stop):
            if self.discrete_actions:
                ...
                if found_goal:
                    dt = xyt_local[0] ** 2 + xyt_local[1] ** 2
                    if dt <= 0.6:
                        print("stop in planner because dt is ",dt)
                        self.next_action = DiscreteNavigationAction.STOP
                        if (np.abs(relative_angle_to_closest_goal) > 60):
                            # action = ContinuousNavigationAction([0, 0, -relative_angle_to_stg]) 
                            action = DiscreteNavigationAction.STOP                       
                        else:
                            action = DiscreteNavigationAction.STOP
            ...
            if self.discrete_actions:
                ...
                np.abs(relative_angle_to_closest_goal) > 15
            ):
                print("fix the angle towards target and stop")
